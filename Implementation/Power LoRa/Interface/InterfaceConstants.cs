namespace Power_LoRa.Interface
{
	class InterfaceConstants
	{
        #region Public constants
        
        public const int WindowMarginX = 12;
		public const int WindowMarginY = 12;

		public const int MessageLabelMaxWidth = 264;

		public const int GroupBoxLocationX = 13;
		public const int GroupBoxLocationY = 13;
		public const int GroupBoxFirstItemY = 23;
		public const int GroupBoxLastItemY = 4;

		public const int LabelLocationX = 8;
		public const int LabelLocationY = 9;
		public const int LabelWidth = 80;
		public const int LabelHeight = 17;
        public const int ShortLabelWidth = 60;

        public const int ListShortColumnWidth = 50;
        public const int ListColumnWidth = 70;
        public const int ListLongColumnWidth = 80;
        public const int ListItemHeight = 17;
        public const int ListHeaderHeight = 24;

        public const int ScrollBarWidth = 20;

        public const int RadioButtonLocationX = 12;
		public const int RadioButtonLocationY = 12;
		public const int RadioButtonHeight = 21;

		public const int ButtonLocationX = 12;
		public const int ButtonLocationY = 12;
		public const int ButtonWidth = 101;
		public const int ButtonHeight = 24;

		public const int InputLocationY = 12;
		public const int InputWidth = 80;
		public const int InputHeight = 24;

		public const int ItemPadding = 4;

		public const int LabelToBoxOffset = 3;
        #endregion
    }
}
