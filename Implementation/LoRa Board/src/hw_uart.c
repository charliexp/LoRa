...
/* Private variables ---------------------------------------------------------*/
/* Uart Handle */
UART_HandleTypeDef DAQ_UartHandle;
/* Receive state */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if (huart->Instance == DAQ_USARTX)
	{
		if (RxReceiveIndex == UART_BUFFSIZE - 1)
			...
			RxReceiveIndex++;
		HAL_UART_Receive_IT(&DAQ_UartHandle, RxBuffer + RxReceiveIndex, 1);
	}
...
void UART_Init(void)
{
  DAQ_UartHandle.Instance        = DAQ_USARTX;
  DAQ_UartHandle.Init.BaudRate   = 9600;
  DAQ_UartHandle.Init.WordLength = UART_WORDLENGTH_8B;
  DAQ_UartHandle.Init.StopBits   = UART_STOPBITS_1;
  DAQ_UartHandle.Init.Parity     = UART_PARITY_EVEN;
  DAQ_UartHandle.Init.HwFlowCtl  = UART_HWCONTROL_NONE;
  DAQ_UartHandle.Init.Mode       = UART_MODE_TX_RX;
  
  if(HAL_UART_Init(&DAQ_UartHandle) != HAL_OK)
  {
...
UartRxState_t UART_Receive(uint8_t *buffer, uint16_t *length, uint8_t terminatorChar)
{
	*length = 0;
	...
	if (HAL_GetTick() - lastSendTime > 5 * 1000)
		HAL_UART_ErrorCallback(&DAQ_UartHandle);
...
void UART_Send(uint8_t *buffer, uint8_t length)
{
	...
	HAL_UART_Transmit(&DAQ_UartHandle, buffer, length, 2000);
}

void UART_DeInit(void)
{
  HAL_UART_DeInit(&DAQ_UartHandle);
}
